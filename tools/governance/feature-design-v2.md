...
| Stage | 내용 | DEX에 미치는 영향 | 산출물 |
|---|---|---|---|
| **S0 Mock** | 목업 (현재) — 데이터 계약(`flowops-data.json` 스키마) 확정 | 없음 | 목업 v2 |
| **S1 읽기 스냅샷** | `Get-FlowOpsData.ps1`이 DEX에서 Inventory API + Dataverse **읽기 전용** 조회 → `flowops-data.json` 생성 → 목업이 실데이터로 렌더 | **없음 (읽기만)** | 어댑터 스크립트 (`scripts/Get-FlowOpsData.ps1`) |
| **S2 전용 환경** | FlowOps 전용 환경 신설 + `fw_*` 테이블 + 수집 흐름(스케줄 읽기) | 없음 (DEX는 소스일 뿐) | Dataverse 솔루션 |
| **S3 쓰기 연동** | 과제 등록/승인/단계 전환 쓰기 + Pipelines 이벤트 수신 | FlowOps 환경에만 쓰기, **DEX에는 여전히 쓰기 없음** | 자동화 흐름 8종 |
| **S4 과제 편입** | 기존 운영 과제에 LogFunc v2 적용 배포 (과제별 점진) | 과제 재배포 필요 — 변경 통제 하에 | 편입 체크리스트 |

원칙: **DEX(기본환경)에는 최후까지 쓰기 없음.** 플랫폼 데이터는 전부 FlowOps 전용 환경에.

### 4.2 데이터 계약 (S0↔S1 인터페이스)

```jsonc
// flowops-data.json — 목업과 어댑터가 공유하는 계약
{
...
### 4.3 S1 어댑터가 호출하는 실제 API

| 계약 필드 | 호출 | 비고 |
|---|---|---|
| `kpi.*`, `inventory[]` | `POST api.powerplatform.com/resourcequery/resources/query` (Inventory API) | SPN, 읽기 전용, 15분 신선도 |
| `tasks[]` (초기: 봇/흐름 목록으로 대체) | DEX Dataverse `GET /bots`, `GET /workflows?category eq 5` | 기존 마이그레이션 툴킷 lib 재사용 |
| `runs[]` (S1에선 flow run 이력) | Flow API `GET .../flows/{id}/runs` (상위 N건) | 스로틀 주의 — 과제당 최근 5건만 |
| `releases[]` | S1에선 빈 배열 (Pipelines 미구성) | S3부터 |
...
| R2 | **Inventory API 신선도(15분)·스키마 변화** | 실시간 화면을 인벤토리로 만들면 최신 아님 / ARG 스키마 필드 추가·변경 | 화면 오표시 | 실시간=LogFunc 데이터, 인벤토리 화면엔 "동기화 시각" 상시 표기, 어댑터에 스키마 버전 체크 |
...
| R6 | **DEX 직접 쓰기 사고** | 어댑터/흐름 버그로 기본환경 데이터 변조 | 운영 과제 영향 | S0~S3 동안 DEX 쓰기 코드 자체를 만들지 않음, SPN에 DEX는 읽기 역할만 부여(최소권한) |
...
| R10 | **계약 불일치(목업↔실데이터)** | 실데이터에 목업이 가정 안 한 값(null 소유자, 한글 깨짐 등) | 화면 깨짐 | 데이터 계약에 null 규칙 명시, 어댑터가 계약 검증 후 출력, 샘플 실데이터로 S1 조기 검증 |
| R11 | **기존 과제 편입 부담** | 운영 중 과제 재배포(LogFunc v2) 리스크 | 운영 장애 | S4를 과제별 점진 적용, 편입 전후 병행 모니터링 1주, 롤백 아카이브 확보 |
...
2. 목업 v2 — 메뉴 v2 반영 + 성과(ROI)·일정보드·큐/머신 화면 추가, "Mock 데이터" 상태 배지
3. `scripts/Get-FlowOpsData.ps1` — S1 읽기 어댑터 (DEX → flowops-data.json, 기존 마이그레이션 lib 재사용)
